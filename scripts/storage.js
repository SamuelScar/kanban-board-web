...
const utilitariosArmazenamento = Kanban.utilitarios;
  ...
  return Boolean(
    cartao &&
      typeof cartao === "object" &&
        ...
        (
          cartao.cor === undefined ||
          utilitariosArmazenamento.normalizarCorHexadecimal(cartao.cor).length > 0
        )
    );
  }
...
function carregarQuadro() {
  try {
    const estadoSerializado = window.localStorage.getItem(CHAVE_ARMAZENAMENTO);
...
function salvarQuadro(estadoQuadro) {
  try {
    window.localStorage.setItem(CHAVE_ARMAZENAMENTO, JSON.stringify(estadoQuadro));
  } catch (erro) {
